...
# Word-by-word navigation with Alt/Option or Ctrl + arrows
...
# On SSH login, jump straight into a tmux session via `tat`
if [[ -n "$SSH_CONNECTION" ]] && [[ -z "$TMUX" ]] && [[ $- == *i* ]] && command -v tat >/dev/null 2>&1; then
  tat
fi
